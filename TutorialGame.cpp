...
int TutorialGame::UpdateGame(float dt) {//testing returning int
	...
	if(goatCharacter->getPlayerProjectile()->GetCanCharge()){
		...
		if (TutorialGame::chargeTimeLimitTest()) {
			TutorialGame::resetChargeTime();
			goatCharacter->getPlayerProjectile()->toggleCanCharge();
		}
	}
...
void TutorialGame::setEnemyGoat(EnemyAI* assignCharcter) {
...
void TutorialGame::movePlayer(playerTracking* unitGoat) {
	...
	if ((Window::GetKeyboard()->KeyPressed(NCL::KeyboardKeys::M)) || (Window::GetKeyboard()->KeyHeld(NCL::KeyboardKeys::M))) {
		goatPhysicsObject->addForceToPosition(100,goatTransform->GetPosition(),Vector3{0,-16,0});
	}

	if ((Window::GetKeyboard()->KeyPressed(NCL::KeyboardKeys::C)) && !(unitGoat->getPlayerProjectile()->GetCanCharge()))  {
		goatPhysicsObject->AddForce(((unitGoat->GetTransform().GetOrientation()) * (Vector3(0, 0, 1)).Normalised()) * -5000.0f);
		unitGoat->getPlayerProjectile()->toggleCanCharge();
	}
