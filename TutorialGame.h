...
namespace NCL {
	namespace CSC8503 {
		...
		public:
			...
			//charge Time
			void addToRunningChargeTime(float timeIncrement) {
				runnigChargeTime += timeIncrement;
			}

			void resetChargeTime() {
				runnigChargeTime = 0.0f;
			}

			bool chargeTimeLimitTest() {
				if (runnigChargeTime >= chargeTimeLimit) {
					resetChargeTime();
					return true;
				}
				return false;
